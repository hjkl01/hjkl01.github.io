...
const data: DataType[] = [
  ...
  },
  {
    ...
    character: "$",
    meaning:
      "匹配输入的结束。如果多行标志被设置为true，那么匹配换行符前的位置。例如，/t$/并不会匹配\"eater\"中的't'，但是会匹配\"eat\"中的't'。",
  },
  {
    key: "4",
    character: "*",
    ...
    <table className="min-w-full bg-white border border-gray-200">
      <thead>
        <tr className="bg-gray-100">
          <th className="py-2 px-4 border-b text-left w-1/5">字符</th>
          <th className="py-2 px-4 border-b text-left w-4/5">含义</th>
        </tr>
      </thead>
      <tbody>
        {data.map((item) => (
          <tr key={item.key} className="hover:bg-gray-50">
            <td className="py-2 px-4 border-b font-mono">{item.character}</td>
...
const RegexApp = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-5 bg-gray-50 rounded-lg shadow">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Regex Online</h1>
      
      <form onSubmit={handleTest} className="space-y-4">
        <div>
          <label className="block text-sm font-bold mb-1">Regular Expression</label>
          <input
            type="text"
            value={regexInput}
      ...
      {matches.length > 0 && (
        <div className="mt-6">
          <h2 className="text-xl font-bold text-gray-800 mb-3">Matches:</h2>
